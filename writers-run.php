...
$text = $_POST ["text"];
$date = date("Y-m-d");
$sql = $pdo -> prepare( "INSERT INTO `writers` (`id`, `name`, `img`, `text`) VALUES (NULL, '$name', '$img', '$text')");
$sql -> execute(array('name' =>$name, 'img'=>$img, 'text'=>$text ));
if (!$sql) {
    echo "<script>alert('Что-то пошло не так')</script>";
} else {  
    header('Location:./writers.php');
    echo "<script>alert('Пост успешно опубликован!')</script>"; 
}
?>